...
export default function Page() {
  ...
  const res = (e)=>{
    e.preventDefault()
    axios.post(`${api}/games`,data).then(res=>res.status===201?setError('Created !'):setError("Error")).catch(err=>setError(err.response.data.message))
  }
/*   const res = (e)=>{
    e.preventDefault()
    fetch(`${api}/games`,{
      ...
      <form className="pad flex flex-col gap-4 text-center " onSubmit={res}>
        ...
        <div className="flex flex-row md:flex-col justify-between items-start">
          <div className="flex flex-col justify-center items-start gap-4">
            ...
            <input className="input w-[300px]" placeholder="Genres" type="text" onChange={(e)=>setGenres(e.target.value.split(','))}/>
            <input className="input w-[300px]" placeholder="Platforms" type="text" onChange={(e)=>setPlatforms(e.target.value.split(','))}/>
            <input className="input w-[110px]" placeholder="Release" type="number" onChange={(e)=>setRelease(e.target.value)}/>
            <input className="input w-[110px]" placeholder="Price" type="number" onChange={(e)=>setPrice(e.target.value)}/>
          </div>

          <div className="flex flex-col justify-center items-start gap-4 w-[30%]">
            <Heading title='requirements' clas="input-headings"/>
            <input className="input w-[320px]" placeholder="min CPU" type="text" onChange={(e)=>setMinCPU(e.target.value)}/>          
            <input className="input w-[320px]" placeholder="min GPU" type="text" onChange={(e)=>setMinGPU(e.target.value)}/>          
            <input className="input w-[140px]" placeholder="min RAM" type="number" onChange={(e)=>setMinRAM(e.target.value)}/>          
            <input className="input w-[140px]" placeholder="min VRAM" type="number" onChange={(e)=>setMinVRAM(e.target.value)}/>
          ...
          <div className="flex flex-col justify-center items-start gap-4 w-[20%]">
            ...
            <label htmlFor="fileUpload" className="input btn-hover">upload image</label>
            <input className="hidden" id="fileUpload" type="file"/>
          </div>
        </div>

        <input className="input self-center btn-hover" type="submit" value='add'/>
      </form>
    </>
